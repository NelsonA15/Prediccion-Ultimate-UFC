...
ufc_data <- ufc_data[-1, ]
colnames(ufc_data)
# Filtrar para incluir solo datos donde gender es 'MALE'
ufc_data_male <- ufc_data %>%
  filter(gender == "MALE")

ufc_pca <- subset(ufc_data_male, select = c(R_ev, B_ev, B_current_lose_streak, Winner,
...
# Dividir los datos en conjunto de entrenamiento (80%) y de validación (20%)
set.seed(123) 
